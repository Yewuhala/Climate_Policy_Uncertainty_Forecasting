...
# ARNN
arnn_1 <- nnetar(train_1$cpu_index, xreg = train_reg_reduced_1, MaxNWts = 5000)
...
ss <- AddAutoAr(ss, train_1$cpu_index)
...
# ARNN
arnn_3 <- nnetar(train_3$cpu_index, xreg = train_reg_reduced_3, MaxNWts = 5500)
...
# ARNN
arnn_6 <- nnetar(train_6$cpu_index, xreg = train_reg_reduced_6, MaxNWts = 5700)
...
# ARNN
arnn_12 <- nnetar(train_12$cpu_index, xreg = train_reg_reduced_12, MaxNWts = 5600)
...
# ARFIMA
arfima_24 <- arfima(train_24$cpu_index, xreg = train_reg_reduced_24[,1:90])
arfima_24_pred <- forecast(arfima_24, h = n, xreg = test_reg_reduced_24[,1:90])
...
# ARIMA
arima_24 <- auto.arima(train_24$cpu_index, xreg = train_reg_reduced_24[,1:90])
arima_24_pred <- forecast(arima_24, h = n, xreg = test_reg_reduced_24[,1:90])
...
# ARNN
arnn_24 <- nnetar(train_24$cpu_index, xreg = train_reg_reduced_24[,1:90], MaxNWts = 5500)
...
bsts_24 <- bsts(cpu_index ~ .,
                ...
                ping = 100,
                seed = 100,
                data = as.data.frame(cbind(train_24$cpu_index, train_reg_reduced_24[,1:90])) %>% rename('cpu_index' = V1))
bsts_24_pred <- predict(bsts_24, horizon = n, burn = SuggestBurn(.1, bsts_24), newdata = test_reg_reduced_24[,1:90])
